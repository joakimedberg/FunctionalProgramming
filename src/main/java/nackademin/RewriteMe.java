...
public class RewriteMe {
    ...
    public List<String> getAllAnswerOptionsDistinct(){
        return questions.stream()
                .flatMap(e -> e.getAllAnswers().stream())
                .distinct()
                .collect(Collectors.toList());
    }
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        return (int) questions.stream()
                .filter(e -> e.getAllAnswers().contains(answerCandidate))
                .count();
    }
    ...
    public Map<Category, List<String>> getQuestionGroupedByCategory(){
        return new HashMap<>(questions.stream()
    ...
    {
        return questions.stream()
                .filter(e -> e.category.equals(c))
                .flatMap(e -> e.getAllAnswers().stream())
                .reduce( (newValue, oldValue) ->  newValue.length() > oldValue.length() ? newValue : oldValue).get();
